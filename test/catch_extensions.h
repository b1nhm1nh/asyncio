...
// This macro unwraps the expected value and compares without Catch2 decomposition.
#define INTERNAL_REQUIRE_EQ(var, eq_var, expr, expected_val)    \
    do {                                                        \
        const auto var = expr;                                  \
        REQUIRE(var.has_value());                               \
        const bool eq_var = (*(var) == (expected_val));         \
        REQUIRE(eq_var);                                        \
    } while(0)

#define REQUIRE_EQ(expr, expected_val) INTERNAL_REQUIRE_EQ(INTERNAL_CATCH_UNIQUE_NAME(_result), INTERNAL_CATCH_UNIQUE_NAME(_eq), expr, expected_val)
